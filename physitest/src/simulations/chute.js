...
SimRunner.register('chute', {
  ...
  computeMetrics(p) {
    ...
    const ec    = 0.5 * vf * vf;
    const amax  = g;
    // Durée de chute libre pure depuis h (comme si v0 = 0)
    const tfree = Math.sqrt(2 * h / g);
    return { tf, vf, hmax, ec, amax, tfree };
  },
  ...
  if (done) {
    ...
    if (running && showVectors && current) {
      ...
      if (vabs > 0.1) {
        ...
        // v_cur > 0 = montée → flèche vers le HAUT (py diminue en canvas)
        // v_cur < 0 = descente → flèche vers le BAS (py augmente en canvas)
        const dyArrow = v_cur > 0 ? -arrowLen : arrowLen;
        const col_v = v_cur > 0 ? '#2dd4a0' : '#05a1f0';
        const ax = cx + r_obj + 6;
        ctx.strokeStyle = col_v; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(ax, py_cur); ctx.lineTo(ax, py_cur + dyArrow); ctx.stroke();
        drawArrow(ctx, ax, py_cur, ax, py_cur + dyArrow, col_v, 7);
        ctx.fillStyle = col_v; ctx.font = '10px monospace';
        ctx.fillText('v=' + vabs.toFixed(1), ax + 4, py_cur + dyArrow / 2);
      }
    }
...
function drawGraph(ctx, gx, gy, gw, gh, state, p, mode) {
  ...
  if (mode === 'y') {
    ...
    // On affiche |v| : la vitesse de chute augmente toujours de 0 à vf
    // (même si v0>0, on montre d'abord la décélération jusqu'à 0, puis l'accélération)
    const vmax_abs = Math.sqrt(disc);
    const toVg = val => py + ph - (Math.max(0, val) / Math.max(vmax_abs, 0.01)) * (ph - 4);

    // Axe zéro en bas
    ctx.strokeStyle = 'rgba(5,161,240,0.12)'; ctx.lineWidth = 0.8;
    ctx.beginPath(); ctx.moveTo(px, py + ph); ctx.lineTo(px + pw, py + ph); ctx.stroke();

    // Courbe |v(t)| — toujours croissante à partir du point le plus bas
    ctx.beginPath();
    hist.forEach((pt, i) => {
      // |v| = valeur absolue de la vitesse algébrique
      const absV = Math.abs(pt.v);
      i === 0 ? ctx.moveTo(toX(pt.t), toVg(absV))
              : ctx.lineTo(toX(pt.t), toVg(absV));
    });
    ctx.strokeStyle = '#05a1f0'; ctx.lineWidth = 2; ctx.stroke();

    // Si v0 > 0 : la courbe descend d'abord (décélération) puis remonte (accélération)
    // Annoter le point de vitesse nulle (sommet)
    if (v0 > 0) {
      const tSommet = v0 / g;
      if (tSommet <= hist[hist.length-1].t) {
        ctx.beginPath(); ctx.arc(toX(tSommet), toVg(0), 4, 0, Math.PI * 2);
        ctx.fillStyle = '#2dd4a0'; ctx.fill();
        ctx.fillStyle = '#2dd4a0'; ctx.font = '9px monospace';
        ctx.fillText('v=0 (sommet)', toX(tSommet) + 6, toVg(0) + 4);
      }
    }

    // Point courant
    const last = hist[hist.length - 1];
    ctx.beginPath(); ctx.arc(toX(last.t), toVg(Math.abs(last.v)), 3.5, 0, Math.PI * 2);
    ctx.fillStyle = '#05a1f0'; ctx.fill();
  ...
  if (mode === 'y') {
    ...
    ctx.fillText(vm.toFixed(1), px-3, py+8);
    ctx.fillText('0', px-3, py+ph);
    ctx.save(); ctx.translate(gx+10, gy+mt+ph/2); ctx.rotate(-Math.PI/2);
    ctx.textAlign='center'; ctx.fillStyle='#3d6080'; ctx.fillText('|v| (m/s)',0,0); ctx.restore();
  } else if (mode === 'energy') {
...
function drawTheoretical(ctx, px, py, pw, ph, p, mode, tf) {
  ...
  if (mode === 'y') {
    ...
    const vm=Math.sqrt(disc);
    const toVg = val => py+ph-(Math.max(0,val)/Math.max(vm,0.01))*(ph-4);
    ctx.beginPath();
    for(let i=0;i<=60;i++){
      const t2=(i/60)*tf;
      const absV=Math.abs(v0-g*t2);
      i===0?ctx.moveTo(toX(t2),toVg(absV)):ctx.lineTo(toX(t2),toVg(absV));
    }
    ctx.strokeStyle='rgba(5,161,240,0.1)'; ctx.lineWidth=1.5; ctx.setLineDash([4,4]); ctx.stroke(); ctx.setLineDash([]);
  }
