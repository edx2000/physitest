...
const UI = (() => {
  ...
  function buildSidebar(data) {
    ...
    const bottomSection = el('div', { class: 'sidebar-bottom' });

    const aboutBtn = el('button', { class: 'nav-item nav-item-sm', 'data-route': 'about' });
    aboutBtn.textContent = 'À propos';
    aboutBtn.addEventListener('click', () => Router.navigate('about'));
    bottomSection.appendChild(aboutBtn);

    const supportBtn = el('button', { class: 'sidebar-support-btn' });
    supportBtn.innerHTML = '☕ Soutenir PhysiTest';
    supportBtn.addEventListener('click', () => Router.navigate('support'));
    bottomSection.appendChild(supportBtn);
  ...
  function buildSimToolbar(simId) {
    ...
    modeBtn.addEventListener('click', () => {
      ...
      // SimRunner.isSciMode est mis à jour par trigger via l'action
      // On lit l'état APRES le toggle
      SimRunner.isSciMode = !SimRunner.isSciMode;
    ...
    speeds.forEach((s, i) => {
      ...
      btn.addEventListener('click', () => {
        ...
        customSpeedInput.placeholder = '×' + s.value;
    ...
    const customSpeedInput = el('input', {
      type: 'number',
      class: 'speed-custom-input',
      placeholder: '×1',
      min: '0.05', max: '20', step: '0.05',
      title: 'Facteur de vitesse personnalisé (ex: 0.1, 3, 10…)'
    });
    customSpeedInput.style.cssText = 'width:52px; padding:4px 6px; font-size:0.75rem; font-family:var(--font-mono); background:var(--bg-raised); border:1px solid var(--border-med); border-radius:var(--radius-sm); color:var(--text-primary); outline:none;';
    customSpeedInput.addEventListener('change', () => {
      const v = parseFloat(customSpeedInput.value);
      if (!isNaN(v) && v > 0 && v <= 20) {
        speedWrap.querySelectorAll('.speed-btn').forEach(b => b.classList.remove('active'));
        SimRunner.trigger('setSpeed', v);
      }
    });
    customSpeedInput.addEventListener('keydown', e => {
      if (e.key === 'Enter') customSpeedInput.blur();
    });
    speedWrap.appendChild(customSpeedInput);
