...
const UI = (() => {
  ...
  function buildSimulator(simData, container) {
    ...
    // Canvas
    const canvasH = simData.id === 'pendule' ? 420 : 320;
    ...
    // Pendule : barre de mode simple/sci + vitesse
    if (simData.id === 'pendule') {
      container.appendChild(buildPenduleToolbar(simData.id));
    }

    // Graph switcher (pendule uniquement, visible en mode sci)
    if (simData.id === 'pendule') {
      const sw = buildGraphSwitcher(simData.id);
      sw.id = 'graph-switcher-pendule';
      sw.style.display = 'none'; // masqué en mode simple
  ...
  // ── Pendule toolbar (mode + vitesse) ──────────────────────
  function buildPenduleToolbar(simId) {
    const bar = el('div', { class: 'pendule-toolbar' });

    // Mode simple ↔ scientifique
    const modeBtn = el('button', { class: 'toolbar-mode-btn', id: 'btn-sci-mode', title: 'Passer en mode scientifique' });
    modeBtn.innerHTML = '🔬 Passer en mode scientifique';
    modeBtn.addEventListener('click', () => {
      SimRunner.trigger('toggleSciMode');
      const isSci = !SimRunner.isSciMode;
      SimRunner.isSciMode = isSci;
      modeBtn.innerHTML = isSci ? '📐 Mode simple' : '🔬 Passer en mode scientifique';
      modeBtn.classList.toggle('active', isSci);
      const sw = document.getElementById('graph-switcher-pendule');
  ...
  function buildExplanationPanel(simData) {
    ...
    btn.addEventListener('click', () => {
      ...
      if (open) setTimeout(() => panel.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 40);
    });
    return wrap;
  }
  ...
  function getControl(simId, ctrlId) {
    const range = document.getElementById(`ctrl-${simId}-${ctrlId}`);
    if (!range) return null;
    // realValue is the authoritative value (may exceed slider range)
    if (range.dataset.realValue !== undefined) {
      const v = parseFloat(range.dataset.realValue);
      if (!isNaN(v)) return v;
    }
